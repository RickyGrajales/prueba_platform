...
<body>
    ...
    <div class="form-container">
        ...
        <form action="{{ route('profesorPBJ.actualizarDatos') }}" method="POST">
            @csrf
            @method('PUT')

            <label for="nombre">Nombre:</label>
            <input type="text" name="nombre" id="nombre" value="{{ old('nombre', Auth::guard('usuariospbj')->user()?->nombre) }}">

            <label for="email">Email:</label>
            <input type="email" name="email" id="email" value="{{ old('email', $profesor->email) }}" required>

            <label for="telefono">Teléfono:</label>
            <input type="text" name="telefono" id="telefono" value="{{ old('telefono', $profesor->telefono) }}">
